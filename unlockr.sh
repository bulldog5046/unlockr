...
sed -i 's/$dbg_str = "getExportFeature start";/return 1;\n    $dbg_str = "getExportFeature start";/' /var/sf/lib/perl/5.10.1/SF/SmartAgentManager.pm
sed -i 's/sub getRegistrationStatus {/sub getRegistrationStatus {\n    return { status => getFakeStatus( $AUTH_STATE_AUTHORIZED, $REG_STATE_REGISTERED ) };/' /var/sf/lib/perl/5.10.1/SF/License/SmartLicense.pm
